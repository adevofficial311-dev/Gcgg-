--[[
	AUTONOMOUS ADAPTIVE PERFORMANCE ENGINE — Roblox LocalScript (v3)
	==========================================================================
	This is a rewrite of v2's decision loop from "heuristic, open-loop" to
	"measured, closed-loop." Same honesty notes as before still apply —
	read them, they haven't changed:

	- Cannot lower ping. Ping is server round-trip time; measured/displayed
	  only.
	- Roblox exposes no real GPU profiler to scripts. What changed in v3 is
	  that the engine no longer ONLY guesses cost from object type/rate —
	  it now measures actual frame-time before and after its own changes
	  and learns from that. That's a real signal (your own frame time),
	  not a fabricated one.

	HOW THE CLOSED LOOP WORKS (this is the actual v3 upgrade):
	1. The continuous intensity controller (fast attack / slow decay, same
	   as v2) still decides HOW MANY objects should be disabled overall,
	   based on smoothed FPS vs target FPS.
	2. WHICH objects get disabled is no longer pure static heuristic cost.
	   Every ~2s, the engine picks a small batch (max 5) of the
	   highest-ranked candidates, disables them, then WAITS ~1.5s and
	   measures the actual frame-time change.
	     - If the measured improvement is negligible, it REVERTS that
	       batch (turns them back on) and deprioritizes them.
	     - If it measurably helped, it keeps them off and boosts their
	       learned-impact score.
	   This is a real test-and-check loop against your own measured frame
	   time — not a bigger guess.
	3. When performance recovers and objects need to be re-enabled, the
	   LEAST-proven-useful ones are restored first (not just reverse
	   order), so genuinely-helpful changes are kept as long as possible.
	4. Sudden frame-time spikes bypass the normal 2s/1.5s cycle — they get
	   an immediate reflex disable of the top-ranked candidates (still
	   measured afterward for learning), because waiting through a full
	   cycle during a stutter isn't acceptable.
	5. A lightweight diagnosis (sustained low FPS vs. spiky-but-otherwise-
	   fine) shifts ranking priority: spike-associated objects are favored
	   during spiky conditions, cost/learned-impact dominates otherwise.
	6. A "Learned Insights" panel shows, in plain language, what the
	   engine has actually found — e.g. "Emitter in Torch: ~4.2ms measured
	   benefit" or "no measurable benefit" — so nothing it does is a black
	   box.

	Global settings (post-processing effects, shadows, streaming radius)
	are still simple threshold-driven like v2 — there's only one
	GlobalShadows property, so "which instance caused it" isn't a
	meaningful question for those; the learning loop applies to the
	object registry (particles/trails/beams/fire/smoke/sparkles), where
	attribution actually means something.

	PLACEMENT: LocalScript under StarterPlayerScripts (or StarterGui), in
	a game you own/develop. Client-side only; no gameplay advantage.
--]]
